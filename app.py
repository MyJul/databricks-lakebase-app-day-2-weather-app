...
def _get_embedding_model():
    """Load and cache the embedding model (expensive operation)."""
...
@app.route("/search")
def search_ui():
    """UI for semantic search over news articles and chunks."""
...
def semantic_search():
    ...
    POST body: {
        ...
        "include_chunks": true  # optional, default false
    }
    
    Returns top matching articles and optionally chunks, ranked by similarity.
    """
    ...
    include_chunks = request.json.get("include_chunks", False)
    
    try:
        # Embed the query using the same model as the notebook
        ...
        # Search article-level embeddings
        article_results = lakebase.run_query(
            f"""
            SELECT 
                e.id,
                e.ticker,
        ...
        # Optionally search chunk-level embeddings
        if include_chunks:
